"""Command-line entry point for syncing market odds into
model_predictions.

Usage:
    python -m app.models.odds_cli sync --start 2024-08-01 --end 2024-09-01
"""
...
def _run_sync(start: str, end: str) -> int:
    ...
    try:
        ...
        summary = sync_and_store_market_data(session, list(fixtures))
    finally:
        session.close()

    logger.info(
        "Market data sync %s..%s: fetched=%s upserted=%s failed=%s",
        start,
        end,
        summary.fetched,
        summary.upserted,
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(description="Sync market odds into model_predictions")
